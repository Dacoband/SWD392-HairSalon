...
export const getAppointments = async () => {
  ...
  const response = await axios.get("https://api.vol-ka.studio/api/v1/appointment/get-all", {
    headers: {
...
export const getMembers = async () => {
  const token = getAuthToken();
  const response = await axios.get("https://api.vol-ka.studio/api/v1/member/all", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const getAppointmentsWithDetails = async () => {
  try {
    ...
    const memberMap = new Map(members.map((member: any) => [member.accountId, member.memberName]));
    const stylistMap = new Map(stylists.map((stylist: any) => [stylist.stylistId, stylist.stylistName]));
